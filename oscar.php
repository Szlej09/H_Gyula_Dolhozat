...
class Oscar
{
    ...
    public function getOscarsById($OId)
    {
        $query="SELECT m_ID, title, m_desc, pic FROM movie WHERE m_ID = ".$OId;
        $dbvez= new DBVezerlo();
        $this->oscars =$dbvez->executeSelectQuery($query);
        $dbvez->closeDB();
        return $this->oscars;
    ...
    public function getOscarsByType($Mt_name)
    {
        ...
        INNER JOIN movie_type ON movie_type.mt_ID=movie.m_type WHERE movie_type.Mt_name='".$Mt_name."'";
        $dbvez=new DBVezerlo();
        $this->oscars=$dbvez->executeSelectQuery($query);
        $dbvez->closeDB();
        return $this->oscars;
